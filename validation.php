...
$cart = $context->cart;
$paystack = Module::getInstanceByName('paystack');

if ($cart->id_customer == 0 or $cart->id_address_delivery == 0 or $cart->id_address_invoice == 0 or !$paystack->active) {
...
foreach (Module::getPaymentModules() as $module) {
    if ($module['name'] == 'paystack') {
        $authorized = true;
        break;
    }
}
if (!$authorized) {
    die($paystack->getTranslator()->trans('This payment method is not available.', array(), 'Modules.EftSecure.Shop'));
}
